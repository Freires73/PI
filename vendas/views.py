...
def venda_produto(request):
    if request.method == "GET":
        ...
        return render(request, 'venda_produto.html', {'categorias': categorias, 'produtos': produtos})
        ...
        return redirect(reverse('venda_produto'))

def vender(request, slug):
    if request.method == "GET":
        produto = Produto.objects.get(slug=slug)
        data = initial=produto.__dict__
        data['categoria'] = produto.categoria.id 
        form = VendasForm(initial=data)
        return render(request, 'vender.html', {'form':form})
    
def add_vender(request, slug):
    if request.method == "GET":
        vendas = vender.objects.all()
        return render(request, 'add_pontocoleta.html',{'vendas': vendas})
    elif request.method == "POST":
        nome = request.POST.get('nome')
        ...
        vendas = vender(nome=nome, endereco=endereco, telefone=telefone, bairro=bairro)
